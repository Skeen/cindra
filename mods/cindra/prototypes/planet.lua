...
local function cindra_map_gen()
  ...
  local mg = {
    ...
    property_expression_names = {
      ...
      -- would wall the narrow traversable band, so the engine strips any cliff placed in
      -- it (they only survived out in the impassable lava region). A thin ribbon-world is
      -- cliff-free by design; the volcanic zones stay flat and workable. (cliff_settings
      -- and cindra_cliff_elevation are gone; terrain.cliff_band survives only as the
      -- volcanic-tile band geometry the burned-rock autoplace reads.)
    },
    ...
    -- would block passage through the ribbon (they only survived out in the impassable
    -- lava region). The thin ribbon-world is cliff-free by design, so no zone is walled
    -- off and the whole ribbon stays workable. A bespoke thin-ribbon cliff treatment (and
    -- the deferred zone-11 ice-mountain cliff) remain future work under ci-70r.
  }
