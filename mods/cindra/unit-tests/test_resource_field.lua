...
-- (ci-fb9): the positional damage_bounds put the heat cap at perp 300 (zones 1-3) and
-- the cold cap at -200 (zone 11). ci-4iw pulls the field edge a further
-- FIELD_DAMAGE_MARGIN (20) into the safe side so noise-BLED lethal tiles near the
-- boundary stay field-free. So stone lives on [-100, 280) and ice on (-180, -100) --
-- reaching INTO the survivable edge margin (zone 4 / zone 10, the "reachable at a
-- cost" edge-push) but never onto the bled lethal cap/wall.
...
test("stone lives on the building ribbon + SAFE hot margin, not on the cold cap", function()
  ...
  assert_true(field.stone_richness(270) > 0, "stone out into the survivable hot margin (edge-push, below the cap)")
  assert_eq(0, field.stone_richness(-200), "no stone on the cold cap")
  assert_eq(0, field.stone_richness(280), "no stone AT the field's hot edge (a margin short of the heat cap, ci-4iw)")
  assert_eq(0, field.stone_richness(300), "no stone in the LETHAL heat zone / lava-crust (perp >= 300)")
  assert_eq(0, field.stone_richness(340), "no stone deep in the heat damage zone")
...
test("stone is richest toward the HOT (safe) edge (edge-pushing)", function()
  assert_true(field.stone_richness(270) > field.stone_richness(0),
...
test("ice lives on the SAFE cold margin only, richer deeper (colder)", function()
  ...
  assert_true(field.ice_richness(-170) > 0, "ice out into the survivable cold margin (edge-push, rough-ice side)")
  assert_eq(0, field.ice_richness(-180), "no ice AT the field's cold edge (a margin short of the cold cap, ci-4iw)")
  assert_eq(0, field.ice_richness(-200), "no ice in the LETHAL cold zone / deep-ice cap (perp <= -200)")
  assert_eq(0, field.ice_richness(-400), "no ice deep in the cold damage zone")
  assert_true(field.ice_richness(-170) > field.ice_richness(-105),
...
test("bands honour a per-zone-width override (settings-driven tuning)", function()
  ...
  -- deep-ice edge (cold_from -300); they are allowed on the rough-ice cold band.
  ...
  -- Ice FIELDS track the moved lethal cap (cold_from -300) minus the margin: cold
  -- edge -300 + 20 = -280, so ice sits on the (moved) survivable cold margin (-280,
  -- -200) but stops a margin short of the deep-ice cap.
  assert_true(field.ice_richness(-250, cfg) > 0, "ice on the (moved, further-out) survivable cold margin")
  assert_eq(0, field.ice_richness(-290, cfg), "no ice in the keep-back margin off the (moved) cold cap (ci-4iw)")
end)
...
test("stone mask spans the building ribbon + survivable hot margin, short of the heat cap", function()
  -- Hot edge = heat cap boundary (perp 300) minus the keep-back margin (20) = 280,
  -- EXCLUSIVE (ci-fb9, ci-4iw): reaches the survivable edge, off the bled lethal cap.
  assert_eq("((0 - x) >= -100) * ((0 - x) < 280)", field.stone_mask_expr(), "default stone band")
end)

test("ice mask covers the survivable cold margin, short of the cold cap", function()
  -- Cold edge = cold cap boundary (perp -200) plus the keep-back margin (20) = -180,
  -- EXCLUSIVE (ci-fb9, ci-4iw): reaches the survivable edge, off the bled smooth-ice.
  assert_eq("((0 - x) < -100) * ((0 - x) > -180)", field.ice_mask_expr(), "default ice band")
end)
