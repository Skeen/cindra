...
--      calcite calcination -> lime + CO2; the CTO bridge CO2 + H2 -> methanol;
...
--   5. BYPRODUCT SINKS exist (oxygen, lime are ventable) so the chain can't
--      deadlock; the fluids are all defined.
...
describe("cindra plastic chain: the three chemistries connect end to end", function()
  ...
  it("1b. calcite calcination: calcite -> lime + carbon dioxide", function()
    ...
    assert.is_not_nil(product(r.products, LIME), "calcination yields lime (CaO)")
...
describe("cindra plastic chain: gated behind the signature aluminium", function()
  it("all recipes are off by default", function()
    local recipes = {
      "cindra-electrolysis", "cindra-calcination", "cindra-methanol-synthesis",
      "cindra-mto", "cindra-polymerisation", CATALYST,
      "cindra-vent-oxygen", "cindra-vent-lime",
    }
    for _, name in ipairs(recipes) do
...
describe("cindra plastic chain: byproduct sinks + fluids exist (no deadlock)", function()
  ...
  it("oxygen and lime are ventable (the two byproducts have sinks)", function()
    ...
    local vl = prototypes.recipe["cindra-vent-lime"]
    assert.is_not_nil(vl, "the lime vent must exist")
    assert.is_true(has_ingredient("cindra-vent-lime", LIME), "the lime vent consumes lime")
    assert.are.equal(0, #vl.products, "discarding lime is a pure sink (no product)")
  end)
end)
