...
describe("setting effect: cindra-ribbon-orientation", function()
  it("bounds the generated world across the axis the setting names", function()
    ...
    -- The lever: the two orientations put the gradient on DIFFERENT world axes, so
    -- the setting genuinely selects between two different worlds.
    assert.are_not.equal(axis.perp_expr(axis.VERTICAL), axis.perp_expr(axis.HORIZONTAL),
      "the two orientations must read different world axes")
