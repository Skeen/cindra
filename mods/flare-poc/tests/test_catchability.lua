...
-- overflow to dump - the disposal decision never disappears at any scale
-- (planet_design.md sec.10 "CRITICAL RULE - the flare must NOT be 100%-catchable").
--
...
-- Build a scaled grid: n panels, plus n/2 capacitors and n/2 batteries (storage
-- scaled WITH the array). Returns the sunward-side panels so we can read the net.
local function scaled(surface, n, x_to)
  H.grid(surface, -8, x_to)
  local row = H.panel_row(surface, n, -6, 6)
  local storage_pairs = math.floor(n / 2)
  local x = -6
  for _ = 1, storage_pairs do
    H.capacitor(surface, { x, -6 }); x = x + 4
  end
  for _ = 1, storage_pairs do
    H.battery(surface, { x, -6 }); x = x + 4
  end
  return row
end
...
describe("catchability", function()
  ...
  it("once storage fills, the entire peak overflows", function()
    ...
    after_ticks(6, function()
      ...
      local info = panels.deficit(s, row[1].electric_network_id, PEAK)
