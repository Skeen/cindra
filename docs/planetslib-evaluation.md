...
**Caveat on evidence:** the above is traced from PlanetsLib's source, not
confirmed in-engine. No Factorio install was reachable from the worktree this
spike ran in (`cindra-test` aborts with "Factorio binary not found"), so the
integration suite — including `tests/test_planetslib_compat.lua`, added here —
could not be executed locally; it was syntax-checked and written against the API
calls the existing `tests/test_planet.lua` already uses. A genuine "Cindra +
PlanetsLib both installed" load test was likewise not performed; that is stage 1
of the migration plan below. The plain-Lua unit suites and
`tests/no-orphan-suites.test.sh` were run and pass.
...
| 1 | `ci-gg3x` | Verify in-engine that Cindra + PlanetsLib load clean together and Cindra does not move on the starmap | low | — |
...
- **Do** keep Cindra provably compatible with PlanetsLib as an installed sibling
  (stage 0, landed; stage 1 to verify in-engine).
